...
- Apostrophes use Modifier Letter Apostrophe (U+02BC) while closing single quotes use Right Single Quotation Mark (U+2019), producing semantically distinct codepoints in almost all cases. Contractions (don't), possessives (dog's), and leading abbreviations ('twas, '99) all output U+02BC. The one ambiguous case is bare trailing possessives like `dogs'`, where the apostrophe is indistinguishable from a closing quote without semantic understanding.
