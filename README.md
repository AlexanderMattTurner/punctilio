...
- `punctilio` is idempotent by design: `transform(transform(text))` always equals `transform(text)`. If performance is critical, set `checkIdempotency: false` to skip the verification pass. The `rehypePunctilio` and `remarkPunctilio` plugins default to `checkIdempotency: false` because the separator count check already guards against corruption, and the double-pass penalty compounds across every block-level element. You can override this by passing `checkIdempotency: true` in the plugin options.
