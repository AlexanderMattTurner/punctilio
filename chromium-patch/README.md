# Chromium Patch: Fold U+02BC in Find-in-Page
...
quotes on the page. However, **U+02BC MODIFIER LETTER APOSTROPHE** is not
included in this folding.

U+02BC is the [Unicode-recommended character][unicode-apostrophe] for
representing the apostrophe where it functions as a letter (e.g. Hawaiʻian,
...
When a page uses U+02BC, a user searching for `don't` (with ASCII `'` or
curly `'`) gets **no match**, which is confusing.
...
Add U+02BC to the existing quote-folding switch in `FoldQuoteMarkOrSoftHyphen`
so it maps to ASCII `'`, just like U+2018, U+2019, and U+05F3 already do.

### Files changed

| File | Change |
|------|--------|
| `third_party/blink/renderer/platform/wtf/text/character_names.h` | Add `kModifierLetterApostrophe = 0x02BC` constant |
| `third_party/blink/renderer/platform/text/unicode_utilities.cc` | Add `case uchar::kModifierLetterApostrophe:` to both folding functions |
| `third_party/blink/renderer/platform/text/unicode_utilities_test.cc` | Add U+02BC to `FoldQuoteMarkOrSoftHyphenTest` |
...
3. Create a new branch: `git new-branch fold-modifier-letter-apostrophe`
...
Adding U+02BC is consistent with the existing approach and improves
find-in-page for any page that uses this character.
